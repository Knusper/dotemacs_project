...
** Init file
   ...
   #+BEGIN_SRC emacs-lisp :tangle no
     ...
     ;; org-mode
     (add-to-list 'load-path (expand-file-name "~/emacs-scripts/org-8.3.6/lisp/"))
     (add-to-list 'load-path "~/emacs-scripts/org-8.3.6/contrib/lisp" t)
     ;; auctex
...
*** Notes on org-mode installation
    ...
    - add ~export INFOPATH=${HOME}/emacs-scripts/org-8.3.4/doc/:${INFOPATH}~ to
...
**** use-package - automatically download and install packages
     ...
     # Now install all packaages that are not on the system using the
     # list in [[List of packages I use]].
...
**** Description and links to homepages of packages 
     
     In this section we list all the packages that work
     "out-of-the-box" when installed from the package archives.  The
     packages requiring some customisation are listed in the [[*Specific settings for some packages using usepackage][next section]].
